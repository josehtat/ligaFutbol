    ...
    def handle(self, *args, **options):
        ...
        for i in range(20):
            ...
            equip = Equip(ciutat=ciutat,nom=nom,
                          lliga=lliga, any_fundacio=any_fundacio)
            #print(equip)
            equip.save()
        ...
        for local in lliga.equips.all():
            for visitant in lliga.equips.all():
                if local!=visitant:
                    ...
                    # Seleccionar jugador d'algun dels dos equips
                    jugador_local = choice(local.jugadors.all())
                    jugador_visitante = choice(visitant.jugadors.all())

                    # Generar goles aleatorios para cada equipo
                    goles_local = randint(0, 10)
                    goles_visitante = randint(0, 10)

                    # Guardar goles
                    for i in range(goles_local):
                        gol = Event(partit=partit, jugador=jugador_local, 
                                    tipus_esdeveniment='gol', minut=randint(1, 90))
                        gol.save()

                    for i in range(goles_visitante):
                        gol = Event(partit=partit, jugador=jugador_visitante, 
                                    tipus_esdeveniment='gol', minut=randint(1, 90))
                        gol.save()
